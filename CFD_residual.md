...
- **Contiunity (连续性方程 / 质量守恒)**: 代表质量守恒定律。它的核心逻辑是：流入一个网格单元的流体质量减去流出的质量，应该等于该网格内质量的变化量。

  重要性：这是最关键的残差。如果 Continuity 残差很高或震荡，说明流场中出现了“质量凭空产生或消失”的错误，计算结果是不可信的。通常它是最难收敛的一项。

- **Energy(能量方程）**：代表能量守恒定律（热力学第一定律）。它计算的是温度场和热传递。

  *CFD 含义*：只有当你开启了能量方程（即仿真涉及传热、燃烧或可压缩流体）时才会出现这一项。

  收敛性：通常能量方程比动量方程更容易收敛，其残差值往往能降得很低（例如 $10^{-6}$）。

- **Tdr & Tke (湍流模型方程)**: 这两项表明你使用的是 RANS（雷诺平均）湍流模型，极有可能是 $k-\epsilon$ 模型（k-epsilon model）。
  - **Tke (Turbulent Kinetic Energy, $k$)**: 湍流由动能。它衡量的是流体中湍流脉动（那些乱七八糟的涡旋）所蕴含的能量强度。Tke 越大，湍流越剧烈。
  - **Tdr (Turbulent Dissipation Rate, $\epsilon$)**: 湍流耗散率。它衡量的是湍流能量转化为热能（由于粘性摩擦）而消失的速率。也就是“涡旋消亡”的速度。
  - ***注意***：如果你用的是 $k-\omega$ 模型，这里可能会显示 Sdr (Specific Dissipation Rate)。看到 Tdr 通常意味着是 $k-\epsilon$ 系列模型。

- **X-momentum, Y-momentum, Z-momentum (动量方程)**: 代表动量守恒定律（牛顿第二定律 $F=ma$ 在流体中的应用，即纳维-斯托克斯方程 N-S 方程）。
  - **X-momentum**: X 方向的速度分量 ($u$) 的守恒情况。
  - **Y-momentum**：Y 方向的速度分量 ($v$) 的守恒情况。
  - **Z-momentum**：Z 方向的速度分量 ($w$) 的守恒情况。
  - 收敛性：这三项代表流体在三个方向上的流动速度计算是否准确。
